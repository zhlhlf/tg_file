...
TGFileBot 是一个基于 Go 的 Telegram 自动下载工具。它支持 Bot 和多个 UserBot 账号协作，提供频道媒体自动下载、强制入群、下载校验和发布构建等能力。
...
- `download.go`：自动下载逻辑
- `download_dict.go`：文件下载落盘逻辑
- `download_relay.go`：Bot 分流下载逻辑
- `verify_hash.go`：下载后校验与坏块修复
- `util.go`：辅助函数
...
- `download.max_caption_length`：文件名中 caption 的最大长度
...
- 下载完成后会校验文件大小，并在可用时按 Telegram 分段哈希修复坏块
